...
generate_pitcher_report <- function(pitcher_data, pitcher_name) {
  ...
  pitch_summary <- map_df(pitch_types, ~calculate_pitch_metrics(pitcher_data, .x))
  
  # Build pitch table
  pitch_table <- "| Pitch Type | Avg Velocity | Velo Range | Usage % |\n|---|---|---|---|\n"
  for (pt in pitch_types) {
    metrics <- calculate_pitch_metrics(pitcher_data, pt)
    pitch_table <- paste0(pitch_table, 
                         "| ", pt, " | ", round(metrics$avg_velocity, 1), 
                         " | ", round(metrics$velo_min, 1), "-", round(metrics$velo_max, 1),
                         " | ", round(metrics$usage_pct, 1), " |\n")
  }
  
  # Replace placeholders
  report <- str_replace_all(report, "\\{\\{PLAYER_NAME\\}\\}", pitcher_name)
  report <- str_replace_all(report, "\\{\\{DATE\\}\\}", format(Sys.Date(), "%B %d, %Y"))
  report <- str_replace_all(report, "\\{\\{PITCH_COUNT\\}\\}", nrow(pitcher_data))
  report <- str_replace_all(report, "| Pitch Type | Avg Velocity | Velo Range | Usage % .*", pitch_table)
...
generate_hitter_report <- function(batter_data, batter_name) {
  ...
  # Replace placeholders
  report <- str_replace_all(report, "\\{\\{PLAYER_NAME\\}\\}", batter_name)
  report <- str_replace_all(report, "\\{\\{DATE\\}\\}", format(Sys.Date(), "%B %d, %Y"))
  report <- str_replace_all(report, "\\{\\{PLATE_APPEARANCES\\}\\}", nrow(batter_data))
  report <- str_replace_all(report, "\\{\\{AVG\\}\\}", round(avg, 3))
  report <- str_replace_all(report, "\\{\\{SUMMARY_NOTES\\}\\}", "Awaiting analysis...")
  report <- str_replace_all(report, "\\{\\{.*?\\}\\}", "[DATA PENDING]")
